...
const createPointEditTemplate = (pointData, offersData, destinationsData, isDisabled) => {
  const {destination, data, type, isSaving, isDeleting} = pointData;
    ...
    <form class="event event--edit" action="#" method="post">
      <header class="event__header">
        ...
        <div class="event__field-group  event__field-group--price">
          ...
          <input class="event__input  event__input--price" id="event-price-1" type="number" min="1" step="1" name="event-price" value="${data.price}" ${isDisabled ? 'disabled' : ''}>
        </div>
...
export default class PointEdit extends SmartView {
  ...
  _dateFromChangeHandler([userDate]) {
    // если пользователь выберет дату начала после даты окончания, дата окончания станет равной дате начала + 1h
